...
func HandleGoogleAuth(ctx *gin.Context) {
	...
	ctx.SetCookie("Oauth-State", state, 60, "/", os.Getenv("BACKEND_DOMAIN"), false, true)
...
func (c *AuthController) HandleGoogleAuthCallback(ctx *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			ctx.SetCookie("Signed-token", helper.CreateSignedToken(user.Email), 60*5, "/", os.Getenv("BACKEND_DOMAIN"), false, true)
